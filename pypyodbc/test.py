...
if __name__ == "__main__":
    ...
    for db_desc, conn, create_table_sql in conxs:
        ...
        cur = conn.cursor()
        cur.execute(u"insert into pypyodbc_test_data values(1,'这是pypyodbc模块 :)',12.3,1234.55,?,'17:31:32','2012-12-23',NULL, ?)", (datetime.datetime.now(),ba))
        longtext = u''.join([u'我在马路边，捡到一分钱。']*2)
        cur.execute("insert into pypyodbc_test_data values (?,?,?,?,?,?,?,NULL,NULL)", \
                                (2, \
                                longtext,\
                                Decimal('1233.4513'), \
                                123.44, \
        ...
        print 'Binary length:'+str(len(field))
